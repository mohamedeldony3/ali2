	...
	$callback = function ($update, $bot) {
      ...
      $accounts = json_decode(file_get_contents('accounts.json'),1);
		if(isset($update->message)){
				...
				$token = $config['token'];
				$Spe = file_get_contents("https://api.telegram.org/bot  توكن/getChatMember?chat_id=@Ramo44&user_id=$chatId");
				$Spe2 = file_get_contents("https://api.telegram.org/bot توكن/getChatMember?chat_id=@Ramo44&user_id=$chatId");
				$url_info = file_get_contents("https://api.telegram.org/bot$token/getMe");
				$json_info = json_decode($url_info);
				$userr = $json_info->result->username;
				$ban = explode("\n",file_get_contents("ban.txt"));
				$rmdon = file_get_contents("$chatId/Ramo44");
...
*/
      ...
      $bot->sendMessage([
              ...
              file_put_contents("$from_id/Ramo44","off");
                   $bot->sendMessage([
                 'chat_id'=>$id,
...
'text'=>"تم حظرك ",
              ...
              $bot->sendphoto([ 'chat_id'=>$chatId,
                  'photo'=>"https://t.me/yffhfhchhf/935",
                   'caption'=>'𓊈 𝑊𝐸𝐿𝐿𝐶𝑀𝐸 𓊉 𓁹 التفعيل بواسطة @Ramo44',
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
          	...
          	if($config['mode'] != null){
                  ...
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
                          ...
                          [['text'=>' قناتي❗','url'=>'t.me/Ramo44'],['text'=>'المطور❔','url'=>'t.me/Ramo44']],
                      ]
                  ])
                  ]);
...
*/
		              ...
		              $bot->sendMessage([
                 'chat_id'=>$chatId,
                  'text'=>"https://t.me/Ramo44/3189",
                  ]);
		              
		              
		              } elseif($data == 'maker' and !in_array($chatId,$ban)){
		              mkdir("$chatId");
		              file_put_contents("$chatId/Ramo44","on1");
     ...
     $bot->sendMessage([
                      ...
                      'text'=>"معلومات البوت 
                      بوت مجاني
                      لشراء ملف البوت تواصل معاء المطور @Ramo44
                      قناتي : @Ramo44",
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
...
*/
          	...
          	file_put_contents("$from_id/1/Ramo44","off");
             $bot->editMessageText([
		                			'chat_id'=>$chatId,
		         		                      'message_id'=>$mid,
